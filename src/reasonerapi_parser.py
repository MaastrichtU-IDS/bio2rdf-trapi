...
from utils import get_sparql_endpoint_url

get_bio2rdf_select_query = """PREFIX rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#>
...
def get_predicates_from_bio2rdf():
...
def reasonerapi_to_sparql(reasoner_query):
    ...
    sparql_query_get_bio2rdf = get_bio2rdf_select_query
    predicate_category = ''
    subject_category = ''
    object_category = ''
    predicate_edge_id = ''
    ...
    for edge_id in query_graph['edges'].keys():
        ...
        sparql_query_get_bio2rdf = sparql_query_get_bio2rdf.replace('?_predicate_category', predicate_category)
        sparql_query_get_bio2rdf = sparql_query_get_bio2rdf.replace('?_subject_category', subject_category)
        sparql_query_get_bio2rdf = sparql_query_get_bio2rdf.replace('?_object_category', object_category)

    # Add LIMIT to the SPARQL query if n_results provided
    if n_results:
        sparql_query_get_bio2rdf = sparql_query_get_bio2rdf + ' LIMIT ' + str(n_results)
    ...
    sparql = SPARQLWrapper(get_sparql_endpoint_url())
    sparql.setReturnFormat(JSON)
    sparql.setQuery(sparql_query_get_bio2rdf)
    sparqlwrapper_results = sparql.query().convert()
    sparql_results = sparqlwrapper_results["results"]["bindings"]
    ...
    # Now iterates the bio2rdf SPARQL query results:
